...
Если EasTok передаёт буквальный шаблон `{gift_name}` вместо названия живого подарка, приложение автоматически берёт настоящий подарок из свежего локального журнала EasTok и запускает нужный эффект.
...
Кнопка `Тест` в EasTok отправляет шаблонный текст `{gift_name}`, а не настоящее название подарка. FunTime Live ответит `200 OK`, но эффект не запустится: для него нет настоящего события подарка. Это нормально.

Некоторые версии EasTok могут отправить такой же буквальный шаблон даже при настоящем подарке. FunTime Live отслеживает свежие события из `%APPDATA%\EasTok\Logs`, находит реальное название подарка и запускает эффект автоматически. Во время стрима EasTok должен быть открыт. До эфира проверяй сами эффекты через **Local gift test**, например `Doughnut`.
...
| Тест EasTok ничего не делает | Так и должно быть: он передаёт шаблон без настоящего события подарка. |
| Живой подарок отправляет `{gift_name}` текстом | Запусти FunTime Live до старта эфира и не закрывай EasTok. В Activity Feed должны появиться `EasTok event queued` и `EasTok log fallback`. |
